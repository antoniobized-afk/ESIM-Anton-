...
- **Дедупликация (один раз):** Чтобы не спамить пользователя, в `Order` добавлены поля `lowTrafficNotifiedAt` и `expiryNotifiedAt`. Уведомление о малом трафике шлётся **один раз на eSIM** (флаг `lowTrafficNotifiedAt` сбрасывается в `null` при пополнении в `OrdersService`, после чего возможен повторный сигнал). Уведомление о скором истечении — тоже один раз (`expiryNotifiedAt`). Прежний 24ч-cooldown в `TrafficMonitorService` удалён (продуктовое решение 2026-06-19): он re-send'ил уведомления ежедневно и «плавал» по времени на граничном сравнении с часовым кроном.
