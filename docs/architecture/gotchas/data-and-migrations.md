...
- `prisma migrate dev` — только для локальной disposable БД: если уже применённый локально migration-файл изменить, Prisma обнаружит history conflict/drift и предложит `reset`, который удаляет данные. Для production используется только `prisma migrate deploy`: он применяет pending migrations, не запускает shadow drift-check и не reset-ит БД. Перед production rollout всё равно обязателен backup + `prisma migrate status`, а уже применённые в production migration-файлы нельзя править задним числом.
...
- Phase 20 Step 04 подтвердил, что `users.authProvider/providerId` нельзя
  удалять schema migration-ом, пока не сняты все live dependencies:
  legacy exact-provider fallback/write в `AuthIdentityResolverService`,
  Phase 18 identity backfill source и drift-check в
  `UserMergePreflightService`. До отдельного drop pass поля остаются в schema,
  но не должны возвращаться как UI/API fallback для способов входа.
