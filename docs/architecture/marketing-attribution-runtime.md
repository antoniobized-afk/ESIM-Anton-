...
отдельный product-specific query. Общий лимит — 10 000 data rows; превышение
заканчивается явным `413`, а не silent truncation. Download доступен всем admin
roles под тем же `JwtAdminGuard`; campaign mutations остаются ограничены своей
отдельной role policy.
