...
`loyaltyLevelId` больше нельзя трактовать как authoritative pricing source. Это denormalized projection/read model для совместимости, admin/users listing и быстрых выборок. Если `LoyaltyLevel` с подходящим `minSpent` существует, `loyaltyLevelId` должен указывать на него; `NULL` допустим только когда подходящего уровня реально нет.
...
3. в той же accounting transaction вызывается `LoyaltyService.updateUserLevel()`.
...
Синхронизация `loyaltyLevelId` не является best-effort UI fallback. Если пересчёт уровня после покупки падает, completion accounting должен остаться в `FAILED` и попасть в retry/admin reconciliation, чтобы не оставить тихий drift между `totalSpent` и admin/listing projection.

Исторический drift устраняется Prisma migration, которая идемпотентно пересчитывает `users.loyaltyLevelId` по текущим `users.totalSpent` и `loyalty_levels.minSpent`. Локальные копии production и production база получают этот backfill через обычный migration deploy; локальную БД нельзя переносить в production как источник данных.
